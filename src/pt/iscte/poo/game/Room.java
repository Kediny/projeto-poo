...
public class Room {
    ...
    private void loadRoom(String fileName) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            // Check for the first line to see if it contains a #0; directive
            if ((line = reader.readLine()) != null && line.startsWith("#0;")) {
            ...
            heroStartingPosition = findSpawnPoint(); // No need for filePath anymore
    ...
    // Method to check if a position is within the room's fixed grid boundaries (0 to 9 for both x and y)
    ...
    // Separate function to find the spawn point ('H'), starting from the bottom of the room file
    ...
    // Method to move Manel (the player character) in a specific direction
    public void moveManel(Direction dir) {
        Point2D futurePosition = null;
        switch (dir.toString()) {
            case ("LEFT"):
                futurePosition = new Point2D(manel.getPosition().getX() - 1, manel.getPosition().getY());
                break;
            case ("RIGHT"):
                futurePosition = new Point2D(manel.getPosition().getX() + 1, manel.getPosition().getY());
                break;
            case ("UP"):
                futurePosition = new Point2D(manel.getPosition().getX(), manel.getPosition().getY() - 1);
                break;
            case ("DOWN"):
                futurePosition = new Point2D(manel.getPosition().getX(), manel.getPosition().getY() + 1);
                break;
        ...
        // Add wall checking and move if within room
        if (isWithinRoom(futurePosition)) {
            if (isWall(futurePosition)) {
                return; // Don't move if there's a wall
            }
            manel.move(dir); // Assuming Manel has a `move` method that takes a Direction
        }
    }

    // Method to check if a given position is a wall ('W') in the room grid
    public boolean isWall(Point2D position) {
        return roomGrid[position.getY()][position.getX()] == 'W'; // Check if the position corresponds to a wall
    }
}
